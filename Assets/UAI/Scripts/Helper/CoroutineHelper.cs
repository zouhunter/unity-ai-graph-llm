...
namespace UAI{ 
    ...
    {   
        private static Dictionary<IEnumerator, LitCoroutine> _coroutines = new Dictionary<IEnumerator, LitCoroutine>();
        public static void StartCor(IEnumerator coroutine)
        {
#if UNITY_EDITOR
            if(!EditorApplication.isPlaying)
            {
                var co = new LitCoroutine(coroutine);
                EditorApplication.update += co.Update;
                _coroutines[coroutine] = co;
                return;
            }
#endif
        ...
        {
#if UNITY_EDITOR
            if (!EditorApplication.isPlaying&& _coroutines.TryGetValue(coroutine,out var co))
            {
                EditorApplication.update -= co.Update;
